...
def HS_skipgram(input_file_name, output_file_name1, output_file_name2, embed_size, window_size, embed_epoch, init_lr,
                decay, indi_context):
    ...
    embed_dataset = HSData(sentences=id_sentences, indi_context=indi_context)
    train_set = embed_dataset.get_path_pairs(window_size)  ##获取数据集
    embed_u, embed_w = train_skip(train_set, embed_epoch, init_lr, len(word2id), embed_size)

    # 独立成方法
    file_output = open(output_file_name1, 'w')
    for id, word in id2word.items():
        e = embed_u[id]
        e = ' '.join(map(lambda x: str(x), e))
        file_output.write('%s %s\n' % (word, e))
    file_output.close()
    file_output = open(output_file_name2, 'w')
    for id, word in id2word.items():
        e = embed_w[id]
        e = ' '.join(map(lambda x: str(x), e))
        file_output.write('%s %s\n' % (word, e))
...
if __name__ == '__main__':
    u_embed_mat, w_embed_mat = HS_skipgram(input_file_name, output_file_name1, output_file_name2, embed_size,
                                           window_size, embed_epoch, init_lr, DECAY, indi_context)
