...
class HSData:
    ...
    def __init__(self, sentences, indi_context):  ##数据预处理
        ...
        self.word_freq = self.gen_token_freq()  # 统计词频
    ...
    def get_path_pairs(self, window_size):       ##根据target构造[contest，pos节点，neg节点]序列
        ...
        for sentence in self.sentences:
            for i in range(0, len(sentence) - (2 * window_size + 1) + 1):
                ...
                path_pairs.append([context, pos_path, neg_path])##这个是所有context一起进行更新
        return path_pairs

    def gen_token_freq(self):
        """
        :param sentences:
...
class HS_CBOW:
    def __init__(self,input_file_name, embed_dimension, window_size, epoch, init_lr,
                decay, indi_context):
        self.input_file_name=input_file_name
        self.output_file_name1=output_file_name1
        self.output_file_name2=output_file_name2
    ...
    def HS_cbow(self):
        self.data_prepare()
        self.train_cbow()  ##模型训练

    def train_cbow(self):
        u_embedding = np.random.rand(self.num_vocab, self.embed_dimension)  # 向量初始化  叶子节点的向量   也可高斯分布初始化，但是这边尝试均匀分布效果更好
...
if __name__ == '__main__':
    hs_cbow = HS_CBOW(input_file_name, embed_size, window_size, embed_epoch, init_lr, DECAY,indi_context)
    hs_cbow.save_model( output_file_name1, output_file_name2,)