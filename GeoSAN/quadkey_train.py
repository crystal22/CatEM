...
def evaluate(model, test_dataset, negative_sampler, region_processer, loc2quadkey, device, batch_size=32, num_neg=100,
    ...
    if neg_given:
        loader = DataLoader(test_dataset, batch_size=batch_size,
                            collate_fn=lambda e: collect_fn_neg_quadkey_included(e, test_dataset, region_processer,
                                                                                 loc2quadkey))
    else:
...
def semantic_overlap(model, idx2loc, n):
    ...
    f = open('geosan@100_nyc.txt', 'w', encoding='utf-8')
...
def train(model, train_dataset, test_dataset, optimizer, loss_fn, negative_sampler, test_sampler, region_processer,
    ...
    f = open("results/loss_foursquare.txt", 'wt')
    ...
    # hr, ndcg = evaluate(model, test_dataset, test_sampler, region_processer, loc2quadkey, device, batch_size_test,
    #                     num_neg_test, neg_given=test_neg_given)
    # print("Hit@5: {:.4f}, NDCG@5: {:.4f}".format(hr[4], ndcg[4]))
    # print("Hit@10: {:.4f}, NDCG@10: {:.4f}".format(hr[9], ndcg[9]))
    # if save_results:
    #     with open(save_results, 'wt') as f:
    #         print("{:.4f}".format(match_rate), file=f)
...
if __name__ == "__main__":
    ...
    config = unserialize('./data/config.json')
    ...
    filename_raw = "./data/dataset_UbiComp2016_NYC_for_GeoSAN.txt"
    filename_clean = './data/UbiComp2016_QuadKeyLSBNDataset_NYC.data'
    ...
    if config["train"]["negative_sampler"] in {"PopularitySampler", "KNNPopularitySampler", "KNNSampler",
                                               "KNNWRMFSampler"}:
    ...
    args.save_path = './results/nyc/GeoSAN@' + str(config['model']['hidden_dim_encoder']) + '.pth'
    ...
    if args.load_path:
        ...
        print("=====evaluation=====")
        hr, ndcg = evaluate(model, test_dataset, test_sampler, region_processer, dataset.loc2quadkey, device,
                            config['test']['batch_size'], config['test']['num_negative_samples'],
